...
export default function SliderItem ({ src, title, subtitle, paragraph, children, buttons }: SliderItemProps): JSX.Element {
  return (
    <section className='h-[370px] w-full overflow-hidden relative rounded-b-xl flex items-end' style={{backgroundImage: `linear-gradient(to bottom, rgba(45, 43, 43, 0.624), rgba(35, 32, 32, 0.7)), url(${src})`}}>
      <article className='h-[calc(100%-70px)] zw-[80%] mx-auto flex flex-col justify-center items-center gap-4'>
        <section className='text-center'>
          <h2 className='text-lg sm:text-4xl font-semibold text-gray-200'>{title}</h2>
          <h3 className='text-base sm:text-2xl font-medium text-gray-200'>{subtitle}</h3>
        </section>
        <p className='text-xs sm:text-base text-gray-300 text-center'>{paragraph}</p>
